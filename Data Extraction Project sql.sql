1. --select data that we will use
...
2. -- Looking at Total deaths to Total cases
...
3. --Looking at the Total deaths vs population
...
4. --Looking at Toatal cases vs Population
...
5. --Looking at the country with the highest infection rate compared to Population
...
6. -- Showing countries with the highest deaths count per population
...
7. -- Grouping total deaths by continent
...
select sum(new_cases) as TotalCases, sum(cast(new_deaths as int))as TotalDeaths,(sum(cast(new_deaths as int))/sum(new_cases))*100
  as DeathPercentage--total_deaths,total_cases, (total_deaths/total_cases) *100 as DeathPercentage
from [Portfolio Project].dbo.[Coviddeaths$]
...
11. --creating view to store data for later visualisations
